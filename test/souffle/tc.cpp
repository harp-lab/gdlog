...
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","from\tto"},{"auxArity","0"},{"deliminator","\t"},{"fact-dir","."},{"filename","../../../dataset/vsp_finan512_scagr7-2c_rlfddd/vsp_finan512_scagr7-2c_rlfddd.mtx"},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"from\", \"to\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
signalHandler->setMsg(R"_(path(from,to) :- 
   edge(from,to).
in file tc.dl [9:1-9:34])_");
...
signalHandler->setMsg(R"_(path(from,to) :- 
   path(from,mid),
   path(mid,to).
in file tc.dl [10:1-10:50])_");
if(!(rel_delta_path_23da6daa27e8b245->empty()) && !(rel_path_76cf4ab0e06a645b->empty())) {
[&](){
auto part = rel_delta_path_23da6daa27e8b245->partition();
PARALLEL_START
CREATE_OP_CONTEXT(rel_delta_path_23da6daa27e8b245_op_ctxt,rel_delta_path_23da6daa27e8b245->createContext());
...
auto range = rel_path_76cf4ab0e06a645b->lowerUpperRange_10(Tuple<RamDomain,2>{{ramBitCast(env0[1]), ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},Tuple<RamDomain,2>{{ramBitCast(env0[1]), ramBitCast<RamDomain>(MAX_RAM_SIGNED)}},READ_OP_CONTEXT(rel_path_76cf4ab0e06a645b_op_ctxt));
for(const auto& env1 : range) {
if( !(rel_path_76cf4ab0e06a645b->contains(Tuple<RamDomain,2>{{ramBitCast(env0[0]),ramBitCast(env1[1])}},READ_OP_CONTEXT(rel_path_76cf4ab0e06a645b_op_ctxt))) && !(rel_delta_path_23da6daa27e8b245->contains(Tuple<RamDomain,2>{{ramBitCast(env0[1]),ramBitCast(env1[1])}},READ_OP_CONTEXT(rel_delta_path_23da6daa27e8b245_op_ctxt)))) {
...
signalHandler->setMsg(R"_(path(from,to) :- 
   ...
   path(mid,to).
in file tc.dl [10:1-10:50])_");
if(!(rel_path_76cf4ab0e06a645b->empty()) && !(rel_delta_path_23da6daa27e8b245->empty())) {
[&](){
auto part = rel_path_76cf4ab0e06a645b->partition();
PARALLEL_START
CREATE_OP_CONTEXT(rel_delta_path_23da6daa27e8b245_op_ctxt,rel_delta_path_23da6daa27e8b245->createContext());
CREATE_OP_CONTEXT(rel_new_path_a0453b38c2806942_op_ctxt,rel_new_path_a0453b38c2806942->createContext());
CREATE_OP_CONTEXT(rel_path_76cf4ab0e06a645b_op_ctxt,rel_path_76cf4ab0e06a645b->createContext());
...
rel_new_path_a0453b38c2806942->purge();
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","stdoutprintsize"},{"attributeNames","from\tto"},{"auxArity","0"},{"name","path"},{"operation","printsize"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"from\", \"to\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
Own<t_btree_ii__0_1__11__10::Type> rel_path_76cf4ab0e06a645b;
souffle::RelationWrapper<t_btree_ii__0_1__11__10::Type> wrapper_rel_path_76cf4ab0e06a645b;
...
rel_path_76cf4ab0e06a645b(mk<t_btree_ii__0_1__11__10::Type>()),
wrapper_rel_path_76cf4ab0e06a645b(1, *rel_path_76cf4ab0e06a645b, *this, "path", std::array<const char *,2>{{"i:number","i:number"}}, std::array<const char *,2>{{"from","to"}}, 0),
...
void Sf_tc::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
void Sf_tc::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf_tc::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","stdoutprintsize"},{"attributeNames","from\tto"},{"auxArity","0"},{"name","path"},{"operation","printsize"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"from\", \"to\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf_tc::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","from\tto"},{"auxArity","0"},{"deliminator","\t"},{"fact-dir","."},{"filename","../../../dataset/vsp_finan512_scagr7-2c_rlfddd/vsp_finan512_scagr7-2c_rlfddd.mtx"},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"from\", \"to\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
SymbolTable& Sf_tc::getSymbolTable(){
return symTable;
}

RecordTable& Sf_tc::getRecordTable(){
return recordTable;
}

void Sf_tc::setNumThreads(std::size_t numThreadsValue){
...
void Sf_tc::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
if (name == "edge") {
stratum_edge_31e40d0d94fe9f7c.run(args, ret);
return;}
if (name == "path") {
...
namespace souffle {
SouffleProgram *newInstance_tc(){return new  souffle::Sf_tc;}
SymbolTable *getST_tc(SouffleProgram *p){return &reinterpret_cast<souffle::Sf_tc*>(p)->getSymbolTable();}
} // namespace souffle
...
namespace  souffle {
using namespace souffle;
class factory_Sf_tc: souffle::ProgramFactory {
public:
...
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf_tc::newInstance(){
return new  souffle::Sf_tc();
}

 factory_Sf_tc::factory_Sf_tc():
souffle::ProgramFactory("tc"){
}
...
extern "C" {
souffle::factory_Sf_tc __factory_Sf_tc_instance;
}
#endif
} // namespace souffle
