...
range<t_ind_0::iterator> Type::lowerUpperRange_10(const t_tuple &lower,
                                                  const t_tuple &upper,
                                                  context &h) const {
    t_comparator_0 comparator;
    int cmp = comparator(lower, upper);
...
namespace souffle {
using namespace souffle;
class Stratum_Assign_2b7af4a29bc1821d {
  public:
    Stratum_Assign_2b7af4a29bc1821d(
        SymbolTable &symTable, RecordTable &recordTable,
        ConcurrentCache<std::string, std::regex> &regexCache,
        bool &pruneImdtRels, bool &performIO, SignalHandler *&signalHandler,
...
namespace souffle {
using namespace souffle;
Stratum_Assign_2b7af4a29bc1821d::Stratum_Assign_2b7af4a29bc1821d(
    SymbolTable &symTable, RecordTable &recordTable,
...
void Stratum_Assign_2b7af4a29bc1821d::run(
        ...
        try {
            std::map<std::string, std::string> directiveMap(
                {{"IO", "file"},
                 ...
                 {"deliminator", "\t"},
                 {"fact-dir", "."},
                 {"filename", "/root/workspace/dataset/httpd/assign.facts"},
                 {"name", "Assign"},
                 {"operation", "input"},
            ...
            IOSystem::getInstance()
                ...
                ->readAll(*rel_Assign_fb9d653572c1dfb9);
        } catch (std::exception &e) {
            std::cerr << "Error loading Assign data: " << e.what() << '\n';
            exit(1);
        }
    }
}
...
namespace souffle {
using namespace souffle;
class Stratum_Dereference_c5ce4ed69bccdb11 {
  public:
    Stratum_Dereference_c5ce4ed69bccdb11(
        SymbolTable &symTable, RecordTable &recordTable,
        ConcurrentCache<std::string, std::regex> &regexCache,
        bool &pruneImdtRels, bool &performIO, SignalHandler *&signalHandler,
        std::atomic<std::size_t> &iter, std::atomic<RamDomain> &ctr,
        std::string &inputDirectory, std::string &outputDirectory,
        t_btree_ii__0_1__11__10::Type &rel_Dereference_c8aa981a7766e869);
    void run([[maybe_unused]] const std::vector<RamDomain> &args,
             [[maybe_unused]] std::vector<RamDomain> &ret);
...
namespace souffle {
using namespace souffle;
Stratum_Dereference_c5ce4ed69bccdb11::Stratum_Dereference_c5ce4ed69bccdb11(
    SymbolTable &symTable, RecordTable &recordTable,
    ConcurrentCache<std::string, std::regex> &regexCache, bool &pruneImdtRels,
    bool &performIO, SignalHandler *&signalHandler,
    std::atomic<std::size_t> &iter, std::atomic<RamDomain> &ctr,
    ...
    : symTable(symTable), recordTable(recordTable), regexCache(regexCache),
      ...
      signalHandler(signalHandler), iter(iter), ctr(ctr),
      inputDirectory(inputDirectory), outputDirectory(outputDirectory),
      rel_Dereference_c8aa981a7766e869(&rel_Dereference_c8aa981a7766e869) {}

void Stratum_Dereference_c5ce4ed69bccdb11::run(
    [[maybe_unused]] const std::vector<RamDomain> &args,
    [[maybe_unused]] std::vector<RamDomain> &ret) {
    if (performIO) {
        try {
            std::map<std::string, std::string> directiveMap(
                {{"IO", "file"},
                 ...
                 {"fact-dir", "."},
                 {"filename",
                  "/root/workspace/dataset/httpd/dereference.facts"},
                 {"name", "Dereference"},
                 {"operation", "input"},
                 {"params", "{\"records\": {}, \"relation\": {\"arity\": 2, "
                            "\"params\": [\"src\", \"dest\"]}}"},
            ...
            IOSystem::getInstance()
                .getReader(directiveMap, symTable, recordTable)
                ->readAll(*rel_Dereference_c8aa981a7766e869);
        } catch (std::exception &e) {
            std::cerr << "Error loading Dereference data: " << e.what() << '\n';
            exit(1);
        }
    }
}
...
class Stratum_MemoryAlias_eb2b99d17f3353cf {
  public:
    Stratum_MemoryAlias_eb2b99d17f3353cf(
        SymbolTable &symTable, RecordTable &recordTable,
        ConcurrentCache<std::string, std::regex> &regexCache,
        bool &pruneImdtRels, bool &performIO, SignalHandler *&signalHandler,
        std::atomic<std::size_t> &iter, std::atomic<RamDomain> &ctr,
...
namespace souffle {
using namespace souffle;
Stratum_MemoryAlias_eb2b99d17f3353cf::Stratum_MemoryAlias_eb2b99d17f3353cf(
    SymbolTable &symTable, RecordTable &recordTable,
    ConcurrentCache<std::string, std::regex> &regexCache, bool &pruneImdtRels,
    bool &performIO, SignalHandler *&signalHandler,
    std::atomic<std::size_t> &iter, std::atomic<RamDomain> &ctr,
    ...
    : symTable(symTable), recordTable(recordTable), regexCache(regexCache),
      ...
      signalHandler(signalHandler), iter(iter), ctr(ctr),
      inputDirectory(inputDirectory), outputDirectory(outputDirectory),
      rel_delta_MemoryAlias_a006b6c07c4d7f8c(
          &rel_delta_MemoryAlias_a006b6c07c4d7f8c),
      rel_delta_ValueAlias_ec67472747f9c188(
          &rel_delta_ValueAlias_ec67472747f9c188),
      rel_delta_ValueFlow_66e5c3d04e674156(
          &rel_delta_ValueFlow_66e5c3d04e674156),
      rel_new_MemoryAlias_2c2c590c5354b4cd(
          &rel_new_MemoryAlias_2c2c590c5354b4cd),
...
void Stratum_MemoryAlias_eb2b99d17f3353cf::run(
    [[maybe_unused]] const std::vector<RamDomain> &args,
    [[maybe_unused]] std::vector<RamDomain> &ret) {
    signalHandler->setMsg(R"_(MemoryAlias(x,x) :- 
   Assign(_,x).
in file cspa.dl [38:1-38:35])_");
    if (!(rel_Assign_fb9d653572c1dfb9->empty())) {
        [&]() {
            CREATE_OP_CONTEXT(rel_Assign_fb9d653572c1dfb9_op_ctxt,
                              rel_Assign_fb9d653572c1dfb9->createContext());
            CREATE_OP_CONTEXT(
                rel_MemoryAlias_9a8750c76c041544_op_ctxt,
                rel_MemoryAlias_9a8750c76c041544->createContext());
            for (const auto &env0 : *rel_Assign_fb9d653572c1dfb9) {
                Tuple<RamDomain, 2> tuple{
                    {ramBitCast(env0[1]), ramBitCast(env0[1])}};
                rel_MemoryAlias_9a8750c76c041544->insert(
                    tuple,
                    READ_OP_CONTEXT(rel_MemoryAlias_9a8750c76c041544_op_ctxt));
            }
        }();
    }
    signalHandler->setMsg(R"_(MemoryAlias(x,x) :- 
...
in file cspa.dl [39:1-39:35])_");
    if (!(rel_Assign_fb9d653572c1dfb9->empty())) {
        [&]() {
            ...
            for (const auto &env0 : *rel_Assign_fb9d653572c1dfb9) {
                Tuple<RamDomain, 2> tuple{
                    {ramBitCast(env0[0]), ramBitCast(env0[0])}};
                rel_MemoryAlias_9a8750c76c041544->insert(
                    tuple,
                    READ_OP_CONTEXT(rel_MemoryAlias_9a8750c76c041544_op_ctxt));
            }
...
in file cspa.dl [34:1-34:33])_");
    if (!(rel_Assign_fb9d653572c1dfb9->empty())) {
        [&]() {
            CREATE_OP_CONTEXT(rel_Assign_fb9d653572c1dfb9_op_ctxt,
                              rel_Assign_fb9d653572c1dfb9->createContext());
            CREATE_OP_CONTEXT(rel_ValueFlow_13166cf449369285_op_ctxt,
                              rel_ValueFlow_13166cf449369285->createContext());
            for (const auto &env0 : *rel_Assign_fb9d653572c1dfb9) {
                Tuple<RamDomain, 2> tuple{
                    {ramBitCast(env0[0]), ramBitCast(env0[1])}};
                rel_ValueFlow_13166cf449369285->insert(
                    tuple,
                    READ_OP_CONTEXT(rel_ValueFlow_13166cf449369285_op_ctxt));
            }
        }();
    }
...
in file cspa.dl [36:1-36:33])_");
    ...
    [&]() {
        CREATE_OP_CONTEXT(
            rel_delta_MemoryAlias_a006b6c07c4d7f8c_op_ctxt,
            rel_delta_MemoryAlias_a006b6c07c4d7f8c->createContext());
        CREATE_OP_CONTEXT(rel_MemoryAlias_9a8750c76c041544_op_ctxt,
                          rel_MemoryAlias_9a8750c76c041544->createContext());
        for (const auto &env0 : *rel_MemoryAlias_9a8750c76c041544) {
            Tuple<RamDomain, 2> tuple{
                {ramBitCast(env0[0]), ramBitCast(env0[1])}};
            rel_delta_MemoryAlias_a006b6c07c4d7f8c->insert(
    ...
    [&]() {
        CREATE_OP_CONTEXT(
            rel_delta_ValueAlias_ec67472747f9c188_op_ctxt,
            rel_delta_ValueAlias_ec67472747f9c188->createContext());
        CREATE_OP_CONTEXT(rel_ValueAlias_37462775fdb0331f_op_ctxt,
                          rel_ValueAlias_37462775fdb0331f->createContext());
        for (const auto &env0 : *rel_ValueAlias_37462775fdb0331f) {
            Tuple<RamDomain, 2> tuple{
                {ramBitCast(env0[0]), ramBitCast(env0[1])}};
            rel_delta_ValueAlias_ec67472747f9c188->insert(
                tuple,
                READ_OP_CONTEXT(rel_delta_ValueAlias_ec67472747f9c188_op_ctxt));
        }
    }();
    [&]() {
        CREATE_OP_CONTEXT(
            rel_delta_ValueFlow_66e5c3d04e674156_op_ctxt,
            rel_delta_ValueFlow_66e5c3d04e674156->createContext());
        CREATE_OP_CONTEXT(rel_ValueFlow_13166cf449369285_op_ctxt,
                          rel_ValueFlow_13166cf449369285->createContext());
        for (const auto &env0 : *rel_ValueFlow_13166cf449369285) {
            Tuple<RamDomain, 2> tuple{
                {ramBitCast(env0[0]), ramBitCast(env0[1])}};
            rel_delta_ValueFlow_66e5c3d04e674156->insert(
                tuple,
                READ_OP_CONTEXT(rel_delta_ValueFlow_66e5c3d04e674156_op_ctxt));
        }
    }();
...
in file cspa.dl [29:1-29:77])_");
        if (!(rel_Dereference_c8aa981a7766e869->empty()) &&
            !(rel_delta_ValueAlias_ec67472747f9c188->empty())) {
            [&]() {
                CREATE_OP_CONTEXT(
                    rel_delta_ValueAlias_ec67472747f9c188_op_ctxt,
                    rel_delta_ValueAlias_ec67472747f9c188->createContext());
                CREATE_OP_CONTEXT(
                    rel_new_MemoryAlias_2c2c590c5354b4cd_op_ctxt,
                    rel_new_MemoryAlias_2c2c590c5354b4cd->createContext());
                CREATE_OP_CONTEXT(
                    rel_Dereference_c8aa981a7766e869_op_ctxt,
                    rel_Dereference_c8aa981a7766e869->createContext());
                CREATE_OP_CONTEXT(
                    rel_MemoryAlias_9a8750c76c041544_op_ctxt,
                    rel_MemoryAlias_9a8750c76c041544->createContext());
                for (const auto &env0 : *rel_Dereference_c8aa981a7766e869) {
                    auto range =
                        rel_delta_ValueAlias_ec67472747f9c188->lowerUpperRange_10(
                            Tuple<RamDomain, 2>{
                                {ramBitCast(env0[0]),
                    ...
                    for (const auto &env1 : range) {
                        auto range =
                            rel_Dereference_c8aa981a7766e869->lowerUpperRange_10(
                                Tuple<RamDomain, 2>{
                                    {ramBitCast(env1[1]),
                                     ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},
                                Tuple<RamDomain, 2>{
                        ...
                        for (const auto &env2 : range) {
                            if (!(rel_MemoryAlias_9a8750c76c041544->contains(
                                    Tuple<RamDomain, 2>{{ramBitCast(env0[1]),
                                                         ramBitCast(env2[1])}},
                                    READ_OP_CONTEXT(
                                        rel_MemoryAlias_9a8750c76c041544_op_ctxt)))) {
                                Tuple<RamDomain, 2> tuple{
                                    {ramBitCast(env0[1]), ramBitCast(env2[1])}};
                                rel_new_MemoryAlias_2c2c590c5354b4cd->insert(
                                    tuple,
                                    READ_OP_CONTEXT(
                                        rel_new_MemoryAlias_2c2c590c5354b4cd_op_ctxt));
                            }
...
in file cspa.dl [26:1-26:54])_");
        if (!(rel_delta_ValueFlow_66e5c3d04e674156->empty()) &&
            !(rel_ValueFlow_13166cf449369285->empty())) {
            [&]() {
                CREATE_OP_CONTEXT(
                    rel_delta_ValueFlow_66e5c3d04e674156_op_ctxt,
                    rel_delta_ValueFlow_66e5c3d04e674156->createContext());
                CREATE_OP_CONTEXT(
                    rel_new_ValueAlias_3143e2bd0d62924a_op_ctxt,
                    rel_new_ValueAlias_3143e2bd0d62924a->createContext());
                CREATE_OP_CONTEXT(
                    rel_ValueAlias_37462775fdb0331f_op_ctxt,
                    rel_ValueAlias_37462775fdb0331f->createContext());
                CREATE_OP_CONTEXT(
                    rel_ValueFlow_13166cf449369285_op_ctxt,
                    rel_ValueFlow_13166cf449369285->createContext());
                for (const auto &env0 : *rel_delta_ValueFlow_66e5c3d04e674156) {
                    auto range =
                        rel_ValueFlow_13166cf449369285->lowerUpperRange_10(
                            Tuple<RamDomain, 2>{
                                {ramBitCast(env0[0]),
                                 ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},
                            Tuple<RamDomain, 2>{
                            ...
                            READ_OP_CONTEXT(
                                rel_ValueFlow_13166cf449369285_op_ctxt));
                    for (const auto &env1 : range) {
                        if (!(rel_ValueAlias_37462775fdb0331f->contains(
                                Tuple<RamDomain, 2>{
                                    {ramBitCast(env0[1]), ramBitCast(env1[1])}},
                                READ_OP_CONTEXT(
                                    rel_ValueAlias_37462775fdb0331f_op_ctxt))) &&
                            !(rel_delta_ValueFlow_66e5c3d04e674156->contains(
                                Tuple<RamDomain, 2>{
                            ...
                            Tuple<RamDomain, 2> tuple{
                                {ramBitCast(env0[1]), ramBitCast(env1[1])}};
                            rel_new_ValueAlias_3143e2bd0d62924a->insert(
                                tuple,
                                READ_OP_CONTEXT(
                                    rel_new_ValueAlias_3143e2bd0d62924a_op_ctxt));
                        }
   ...
   ValueFlow(z,y).
in file cspa.dl [26:1-26:54])_");
        if (!(rel_ValueFlow_13166cf449369285->empty()) &&
            !(rel_delta_ValueFlow_66e5c3d04e674156->empty())) {
            [&]() {
                CREATE_OP_CONTEXT(
                    rel_delta_ValueFlow_66e5c3d04e674156_op_ctxt,
                    rel_delta_ValueFlow_66e5c3d04e674156->createContext());
                ...
                for (const auto &env0 : *rel_ValueFlow_13166cf449369285) {
                    auto range =
                        rel_delta_ValueFlow_66e5c3d04e674156->lowerUpperRange_10(
                            Tuple<RamDomain, 2>{
                                {ramBitCast(env0[0]),
                                 ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},
                            Tuple<RamDomain, 2>{
   ...
   MemoryAlias(z,w),
   ValueFlow(w,y).
in file cspa.dl [31:1-31:72])_");
        if (!(rel_MemoryAlias_9a8750c76c041544->empty()) &&
            !(rel_ValueFlow_13166cf449369285->empty()) &&
            !(rel_delta_ValueFlow_66e5c3d04e674156->empty())) {
            [&]() {
                CREATE_OP_CONTEXT(
                ...
                for (const auto &env0 : *rel_delta_ValueFlow_66e5c3d04e674156) {
                    auto range =
                        rel_MemoryAlias_9a8750c76c041544->lowerUpperRange_10(
                            Tuple<RamDomain, 2>{
                                {ramBitCast(env0[0]),
                                 ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},
                            Tuple<RamDomain, 2>{
                            ...
                            READ_OP_CONTEXT(
                                rel_MemoryAlias_9a8750c76c041544_op_ctxt));
                    for (const auto &env1 : range) {
                        if (!(rel_delta_MemoryAlias_a006b6c07c4d7f8c->contains(
                                Tuple<RamDomain, 2>{
                                    {ramBitCast(env0[0]), ramBitCast(env1[1])}},
                                READ_OP_CONTEXT(
                                    rel_delta_MemoryAlias_a006b6c07c4d7f8c_op_ctxt)))) {
                            auto range =
                                rel_ValueFlow_13166cf449369285->lowerUpperRange_10(
                                    Tuple<RamDomain, 2>{{ramBitCast(env1[1]),
...
in file cspa.dl [31:1-31:72])_");
        if (!(rel_ValueFlow_13166cf449369285->empty()) &&
            !(rel_delta_MemoryAlias_a006b6c07c4d7f8c->empty())) {
            [&]() {
                CREATE_OP_CONTEXT(
                    rel_delta_MemoryAlias_a006b6c07c4d7f8c_op_ctxt,
                    rel_delta_MemoryAlias_a006b6c07c4d7f8c->createContext());
                ...
                for (const auto &env0 : *rel_ValueFlow_13166cf449369285) {
                    auto range =
                        rel_delta_MemoryAlias_a006b6c07c4d7f8c->lowerUpperRange_10(
                            Tuple<RamDomain, 2>{
                                {ramBitCast(env0[0]),
                                 ramBitCast<RamDomain>(MIN_RAM_SIGNED)}},
                            Tuple<RamDomain, 2>{
        ...
        if (!(rel_delta_ValueFlow_66e5c3d04e674156->empty()) &&
            ...
            [&]() {
                CREATE_OP_CONTEXT(
                    ...
                    rel_delta_ValueFlow_66e5c3d04e674156->createContext());
                CREATE_OP_CONTEXT(
                    rel_new_ValueFlow_5e353ff29d46b279_op_ctxt,
                    rel_new_ValueFlow_5e353ff29d46b279->createContext());
                CREATE_OP_CONTEXT(
                    rel_ValueFlow_13166cf449369285_op_ctxt,
                    rel_ValueFlow_13166cf449369285->createContext());
                for (const auto &env0 : *rel_delta_ValueFlow_66e5c3d04e674156) {
                    auto range =
                        rel_ValueFlow_13166cf449369285->lowerUpperRange_10(
                            ...
                            READ_OP_CONTEXT(
                                rel_ValueFlow_13166cf449369285_op_ctxt));
                    for (const auto &env1 : range) {
                        if (!(rel_ValueFlow_13166cf449369285->contains(
                                Tuple<RamDomain, 2>{
                                    {ramBitCast(env0[0]), ramBitCast(env1[1])}},
                                READ_OP_CONTEXT(
                                    rel_ValueFlow_13166cf449369285_op_ctxt))) &&
                            ...
                            Tuple<RamDomain, 2> tuple{
                                {ramBitCast(env0[0]), ramBitCast(env1[1])}};
                            rel_new_ValueFlow_5e353ff29d46b279->insert(
                                tuple,
                                READ_OP_CONTEXT(
                                    rel_new_ValueFlow_5e353ff29d46b279_op_ctxt));
                        }
...
in file cspa.dl [27:1-27:52])_");
        ...
        if (rel_new_MemoryAlias_2c2c590c5354b4cd->empty() &&
            rel_new_ValueAlias_3143e2bd0d62924a->empty() &&
            rel_new_ValueFlow_5e353ff29d46b279->empty())
            break;
        [&]() {
            CREATE_OP_CONTEXT(
                rel_new_MemoryAlias_2c2c590c5354b4cd_op_ctxt,
        ...
        std::swap(rel_delta_MemoryAlias_a006b6c07c4d7f8c,
                  rel_new_MemoryAlias_2c2c590c5354b4cd);
        ...
        [&]() {
            ...
            for (const auto &env0 : *rel_new_ValueAlias_3143e2bd0d62924a) {
                Tuple<RamDomain, 2> tuple{
                    {ramBitCast(env0[0]), ramBitCast(env0[1])}};
                rel_ValueAlias_37462775fdb0331f->insert(
                    tuple,
                    READ_OP_CONTEXT(rel_ValueAlias_37462775fdb0331f_op_ctxt));
            }
        }();
        std::swap(rel_delta_ValueAlias_ec67472747f9c188,
                  rel_new_ValueAlias_3143e2bd0d62924a);
        ...
        std::swap(rel_delta_ValueFlow_66e5c3d04e674156,
                  rel_new_ValueFlow_5e353ff29d46b279);
        rel_new_ValueFlow_5e353ff29d46b279->purge();
        loop_counter = (ramBitCast<RamUnsigned>(loop_counter) +
                        ramBitCast<RamUnsigned>(RamUnsigned(1)));
        iter++;
    }
    iter = 0;
    ...
    if (performIO) {
        try {
            ...
            IOSystem::getInstance()
                ...
                ->writeAll(*rel_MemoryAlias_9a8750c76c041544);
        } catch (std::exception &e) {
            std::cerr << e.what();
            exit(1);
        }
    ...
    if (performIO) {
        try {
            ...
            IOSystem::getInstance()
                .getWriter(directiveMap, symTable, recordTable)
                ->writeAll(*rel_ValueAlias_37462775fdb0331f);
        } catch (std::exception &e) {
            std::cerr << e.what();
            exit(1);
        }
    ...
    if (performIO) {
        try {
            ...
            IOSystem::getInstance()
                .getWriter(directiveMap, symTable, recordTable)
                ->writeAll(*rel_ValueFlow_13166cf449369285);
        } catch (std::exception &e) {
            std::cerr << e.what();
            exit(1);
        }
    ...
    if (pruneImdtRels)
        rel_Assign_fb9d653572c1dfb9->purge();
    if (pruneImdtRels)
        rel_Dereference_c8aa981a7766e869->purge();
}
...
namespace souffle {
using namespace souffle;
Stratum_tmp_ff6f1a280c21bb7c::Stratum_tmp_ff6f1a280c21bb7c(
    SymbolTable &symTable, RecordTable &recordTable,
    ConcurrentCache<std::string, std::regex> &regexCache, bool &pruneImdtRels,
    bool &performIO, SignalHandler *&signalHandler,
    std::atomic<std::size_t> &iter, std::atomic<RamDomain> &ctr,
...
void Stratum_tmp_ff6f1a280c21bb7c::run(
    [[maybe_unused]] const std::vector<RamDomain> &args,
    [[maybe_unused]] std::vector<RamDomain> &ret) {
    if (performIO) {
        try {
            ...
            IOSystem::getInstance()
                .getWriter(directiveMap, symTable, recordTable)
                ->writeAll(*rel_tmp_033a558889f5439d);
        } catch (std::exception &e) {
            std::cerr << e.what();
            exit(1);
        }
...
class Sf_cspa : public SouffleProgram {
  ...
  private:
    ...
    souffle::RelationWrapper<t_btree_ii__0_1__11::Type>
        wrapper_rel_Assign_fb9d653572c1dfb9;
    Own<t_btree_ii__0_1__11__10::Type> rel_Dereference_c8aa981a7766e869;
    souffle::RelationWrapper<t_btree_ii__0_1__11__10::Type>
        wrapper_rel_Dereference_c8aa981a7766e869;
    Own<t_btree_ii__0_1__11__10::Type> rel_MemoryAlias_9a8750c76c041544;
    souffle::RelationWrapper<t_btree_ii__0_1__11__10::Type>
        wrapper_rel_MemoryAlias_9a8750c76c041544;
    ...
    souffle::RelationWrapper<t_btree_ii__0_1__11::Type>
        wrapper_rel_ValueAlias_37462775fdb0331f;
    ...
    souffle::RelationWrapper<t_btree_ii__0_1__11__10::Type>
        wrapper_rel_ValueFlow_13166cf449369285;
    ...
    souffle::RelationWrapper<t_btree_ii__0_1__11::Type>
        wrapper_rel_tmp_033a558889f5439d;
    Stratum_Assign_2b7af4a29bc1821d stratum_Assign_0260c34870780048;
    Stratum_Dereference_c5ce4ed69bccdb11 stratum_Dereference_bbb072ef70dcb2d6;
...
Sf_cspa::Sf_cspa()
    : symTable(), recordTable(), regexCache(),
      ...
      wrapper_rel_Assign_fb9d653572c1dfb9(
          0, *rel_Assign_fb9d653572c1dfb9, *this, "Assign",
          std::array<const char *, 2>{{"i:number", "i:number"}},
          std::array<const char *, 2>{{"src", "dest"}}, 0),
      rel_Dereference_c8aa981a7766e869(mk<t_btree_ii__0_1__11__10::Type>()),
      wrapper_rel_Dereference_c8aa981a7766e869(
          1, *rel_Dereference_c8aa981a7766e869, *this, "Dereference",
          std::array<const char *, 2>{{"i:number", "i:number"}},
          std::array<const char *, 2>{{"src", "dest"}}, 0),
      rel_MemoryAlias_9a8750c76c041544(mk<t_btree_ii__0_1__11__10::Type>()),
      wrapper_rel_MemoryAlias_9a8750c76c041544(
          2, *rel_MemoryAlias_9a8750c76c041544, *this, "MemoryAlias",
          std::array<const char *, 2>{{"i:number", "i:number"}},
          std::array<const char *, 2>{{"src", "dest"}}, 0),
      rel_delta_MemoryAlias_a006b6c07c4d7f8c(
          mk<t_btree_ii__0_1__11__10::Type>()),
      ...
      wrapper_rel_ValueAlias_37462775fdb0331f(
          3, *rel_ValueAlias_37462775fdb0331f, *this, "ValueAlias",
          std::array<const char *, 2>{{"i:number", "i:number"}},
          std::array<const char *, 2>{{"src", "dest"}}, 0),
      rel_delta_ValueAlias_ec67472747f9c188(
          mk<t_btree_ii__0_1__11__10::Type>()),
      ...
      wrapper_rel_ValueFlow_13166cf449369285(
          4, *rel_ValueFlow_13166cf449369285, *this, "ValueFlow",
          std::array<const char *, 2>{{"i:number", "i:number"}},
          std::array<const char *, 2>{{"src", "dest"}}, 0),
      ...
      wrapper_rel_tmp_033a558889f5439d(
          5, *rel_tmp_033a558889f5439d, *this, "tmp",
          std::array<const char *, 2>{{"i:number", "i:number"}},
          std::array<const char *, 2>{{"src", "dest"}}, 0),
      stratum_Assign_0260c34870780048(
          symTable, recordTable, regexCache, pruneImdtRels, performIO,
          signalHandler, iter, ctr, inputDirectory, outputDirectory,
          *rel_Assign_fb9d653572c1dfb9),
      stratum_Dereference_bbb072ef70dcb2d6(
          symTable, recordTable, regexCache, pruneImdtRels, performIO,
          signalHandler, iter, ctr, inputDirectory, outputDirectory,
          *rel_Dereference_c8aa981a7766e869),
      stratum_MemoryAlias_09e3f01dfdc49ff9(
          ...
          *rel_new_MemoryAlias_2c2c590c5354b4cd,
          *rel_new_ValueAlias_3143e2bd0d62924a,
          *rel_new_ValueFlow_5e353ff29d46b279, *rel_Assign_fb9d653572c1dfb9,
          *rel_Dereference_c8aa981a7766e869, *rel_MemoryAlias_9a8750c76c041544,
          *rel_ValueAlias_37462775fdb0331f, *rel_ValueFlow_13166cf449369285),
      stratum_tmp_88325b656f03e9c6(symTable, recordTable, regexCache,
                                   pruneImdtRels, performIO, signalHandler,
                                   iter, ctr, inputDirectory, outputDirectory,
                                   *rel_tmp_033a558889f5439d) {
    addRelation("Assign", wrapper_rel_Assign_fb9d653572c1dfb9, true, false);
    addRelation("Dereference", wrapper_rel_Dereference_c8aa981a7766e869, true,
                false);
    addRelation("MemoryAlias", wrapper_rel_MemoryAlias_9a8750c76c041544, false,
                true);
    addRelation("ValueAlias", wrapper_rel_ValueAlias_37462775fdb0331f, false,
                true);
    addRelation("ValueFlow", wrapper_rel_ValueFlow_13166cf449369285, false,
                true);
...
void Sf_cspa::runFunction(std::string inputDirectoryArg,
                          std::string outputDirectoryArg, bool performIOArg,
                          bool pruneImdtRelsArg) {
    ...
    // -- query evaluation --
    {
        ...
        stratum_Assign_0260c34870780048.run(args, ret);
    }
    {
        std::vector<RamDomain> args, ret;
        stratum_Dereference_bbb072ef70dcb2d6.run(args, ret);
    }
    {
        std::vector<RamDomain> args, ret;
...
void Sf_cspa::run() { runFunction("", "", false, false); }

void Sf_cspa::runAll(std::string inputDirectoryArg,
                     std::string outputDirectoryArg, bool performIOArg,
                     bool pruneImdtRelsArg) {
    ...
    try {
        std::map<std::string, std::string> directiveMap(
            {{"IO", "file"},
             ...
             {"deliminator", "\t"},
             {"fact-dir", "."},
             {"filename", "/root/workspace/dataset/httpd/dereference.facts"},
             {"name", "Dereference"},
             {"operation", "input"},
             {"params", "{\"records\": {}, \"relation\": {\"arity\": 2, "
                        "\"params\": [\"src\", \"dest\"]}}"},
...
void Sf_cspa::dumpInputs() {
    try {
        ...
        IOSystem::getInstance()
            ...
            ->writeAll(*rel_Assign_fb9d653572c1dfb9);
    } catch (std::exception &e) {
        std::cerr << e.what();
        exit(1);
    }
    try {
        ...
        IOSystem::getInstance()
            .getWriter(rwOperation, symTable, recordTable)
            ->writeAll(*rel_Dereference_c8aa981a7766e869);
    } catch (std::exception &e) {
        std::cerr << e.what();
        exit(1);
    }
...
SymbolTable &Sf_cspa::getSymbolTable() { return symTable; }

RecordTable &Sf_cspa::getRecordTable() { return recordTable; }
...
void Sf_cspa::executeSubroutine(std::string name,
...
SymbolTable *getST_cspa(SouffleProgram *p) {
    return &reinterpret_cast<souffle::Sf_cspa *>(p)->getSymbolTable();
}
} // namespace souffle
...
int main(int argc, char **argv) {
    try {
        souffle::CmdOptions opt(R"(./cspa.dl)", R"()", R"()", false, R"()", 1);
...
factory_Sf_cspa::factory_Sf_cspa() : souffle::ProgramFactory("cspa") {}
